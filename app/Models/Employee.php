...
class Employee extends Model
{
    ...
    */
    ...
    protected $fillable = [
        ...
        'tenant_uuid',
        'hired_at',
        'status',  // Added for active/inactive toggle
    ...
    */
    ...
    /**
     * Get toggle status button HTML
     */
    public function getToggleStatusButton()
    {
        $status = $this->status ?? 'active';
        $isActive = $status === 'active';
        $btnClass = $isActive ? 'btn-danger' : 'btn-success';
        $icon = $isActive ? 'la-ban' : 'la-check-circle';
        $text = $isActive ? 'Deactivate' : 'Activate';
        
        return '<button type="button" 
                   class="btn btn-sm '.$btnClass.' toggle-status-btn" 
                   data-employee-id="'.$this->id.'" 
                   data-url="'.route('admin.employee.toggle-status', $this->id).'" 
                   title="'.$text.' Employee">
                    <i class="la '.$icon.'"></i> '.$text.'
                </button>
                <script>
                $(document).ready(function() {
                    $(".toggle-status-btn").off("click").on("click", function(e) {
                        ...
                        $.ajax({
                            ...
                            error: function() {
                                new Noty({ type: "error", text: "Error toggling status" }).show();
                            }
                        });
                    });
                });
    ...
    /**
     * Get ID card button HTML
     */
    public function getIdCardButton()
    {
        return '<div class="btn-group" role="group">
                    <a href="'.route('admin.employee.preview-id-card', $this->id).'" 
                       class="btn btn-sm btn-info" 
                       target="_blank"
                       title="Preview ID Card">
                        <i class="la la-eye"></i> Preview
                    </a>
                    <a href="'.route('admin.employee.generate-id-card', $this->id).'" 
