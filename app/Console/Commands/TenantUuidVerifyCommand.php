...
class TenantUuidVerifyCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $orphanClients = DB::table('clients')->whereNotExists(function ($q) {
            // no DB::raw required here; the whereNotExists closure can simply
            // point to the tenants table and compare columns using whereColumn
            $q->from('tenants')->whereColumn('tenants.id', 'clients.tenant_id');
        })->count();
