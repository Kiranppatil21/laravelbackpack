...
class ClientInvoiceCrudController extends CrudController
{
    ...
    public function getEmployeeAttendance(Request $request)
    {
        ...
        try {
            // Get client with designation rates
            $client = Client::with('designationRates')->findOrFail($clientId);
            ...
            foreach ($employees as $employee) {
                ...
                // Get rates from client's designation-specific rates
                $dailyRate = 0;
                $overtimeRate = 0;
                
                if ($employee->designation) {
                    $designationRate = $client->getRateForDesignation($employee->designation);
                    if ($designationRate) {
                        // Use client rate (what client pays)
                        $dailyRate = $designationRate->client_rate_per_day;
                        $overtimeRate = $designationRate->client_ot_rate_per_hour;
                    }
                }
                
                // Fallback to calculated rate from monthly salary if no designation rate found
                if ($dailyRate == 0 && $employee->monthly_salary) {
                    $dailyRate = $employee->monthly_salary / 30;
                    $overtimeRate = ($dailyRate / 8) * 1.5; // Per hour rate with 1.5x multiplier
                }
                ...
                $attendanceData[] = [
                    ...
                    'overtime_rate' => round($overtimeRate, 2), // Per hour
